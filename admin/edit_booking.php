...
<main>
    <div class="card">
        <a href="user_details.php?id=<?= $booking['user_id'] ?>" style="text-decoration: none; align-self: flex-start; margin-bottom: 16px;">
            <md-text-button>
                <span class="material-symbols-outlined" slot="icon">arrow_back</span>
                Zurück zu <?= htmlspecialchars($booking['firstname']) ?>
            </md-text-button>
        </a>
        <h1 style="margin-top:0;">Buchung bearbeiten</h1>
        <p style="margin-top:-16px; margin-bottom: 24px; color: var(--md-sys-color-on-surface-variant);">
            Buchung vom <?= date('d.m.Y H:i', strtotime($booking['booking_time'])) ?>
        </p>
        ...
        <form action="update_booking.php" method="POST" style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <md-outlined-text-field label="Anzahl Kaffee" type="number" name="quantity" min="1" required value="<?= $booking['quantity'] ?>"></md-outlined-text-field>
